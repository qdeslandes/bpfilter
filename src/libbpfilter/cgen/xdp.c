...
static int _bf_xdp_gen_inline_prologue(struct bf_program *program)
{
    ...
    /* data and data_end are needed only when the ethertype must be read
     * (needs_l3_proto) or when pkt_size must be computed (needs_pkt_size).
     * When neither applies, skip the loads entirely and zero pkt_size. */
    if (program->runtime.needs_l3_proto || program->runtime.needs_pkt_size) {
        ...
        if (program->runtime.needs_l3_proto) {
             ...
             * frames. Only needed when we will dereference packet memory to
             * read the ethertype. R4 is a scratch register for the check. R7
             * is callee-saved and will hold the ethertype across the kfunc
             * call. */
        ...
        /* Compute pkt_size = data_end - data, or zero it if unused. */
        ...
        /* Neither ethertype nor pkt_size needed - just zero pkt_size. */
        EMIT(program,
             BPF_ST_MEM(BPF_DW, BPF_REG_10, BF_PROG_CTX_OFF(pkt_size), 0));
    }

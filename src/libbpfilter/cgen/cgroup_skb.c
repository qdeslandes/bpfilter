...
static int _bf_cgroup_skb_gen_inline_prologue(struct bf_program *program)
{
    ...
    // Copy the packet size (+ETH_HLEN) into the runtime context (or zero it if unused)
     ...
     * network byte order. Load it directly into R7 for the downstream
     * bf_stub_parse_l3_hdr dispatch when needed. */

...
#include "elfstub_pvt.h"
#include "core/logger.h"
#include "core/helper.h"
#include "core/btf.h"

static_assert(ARRAY_SIZE(_bf_rawstubs) == _BF_ELFSTUB_MAX, "_bf_rawstubs doesn't contain as many entries as bf_elfstub_id");

static int _bf_elfstub_prepare(struct bf_elfstub *stub, const struct bf_rawstub *raw)
{
    ...
    for (int i = 0; i < ehdr->e_shnum; i++) {
        if (!bf_streq(&strtab[shdrs[i].sh_name], ".text")) {
            bf_info("section header name is '%s', not '.text.'",
            &strtab[shdrs[i].sh_name]);
            continue;
        }
        ...
        for (size_t j = 0; j < rel_count; j++) {
            ...
            if (type == R_BPF_64_32 && sym_idx < sym_count) {
                ...
                if (name_idx < symstrtab->sh_size) {
                    ...
                    stub->insns[idx] = ((struct bpf_insn) {.code = BPF_JMP | BPF_CALL,
                                        .dst_reg = 0,
                                        .src_reg = BPF_PSEUDO_KFUNC_CALL,
                                        .off = 0,
                                        .imm = id});

                    bf_dbg("updated stub to call '%s' from instruction %lu", name, idx);
                }
            }
        }
    }

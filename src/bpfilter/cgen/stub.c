...
int bf_stub_parse_l3_hdr(struct bf_program *program)
{
...

    {
        ...
        _clean_bf_jmpctx_ struct bf_jmpctx _ = bf_jmpctx_get(program, BPF_JMP_IMM(BPF_JNE, BPF_REG_7, htobe16(ETH_P_IPV6), 0));
        ...
        tcpjmp = bf_jmpctx_get(program, BPF_JMP_IMM(BPF_JEQ, BPF_REG_8, IPPROTO_TCP, 0));
        udpjmp = bf_jmpctx_get(program, BPF_JMP_IMM(BPF_JEQ, BPF_REG_8, IPPROTO_UDP, 0));
        ...
        EMIT(program, BPF_ALU64_IMM(BPF_ADD, BPF_REG_2, sizeof(struct ipv6hdr)));
        EMIT(program, BPF_STX_MEM(BPF_W, BPF_REG_10, BPF_REG_2, BF_PROG_CTX_OFF(l4_offset)));

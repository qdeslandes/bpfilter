...
::std::tuple<::std::string, ::std::string> run(::std::string bin, ::std::vector<::std::string> args)
{
    int out, err;
    ::std::cout << "execcc" << ::std::endl;
    int fd = execccc(bin, args, &out, &err);
    ::std::cout << "exec done" << ::std::endl;
    ::std::string log, logerr;
    ...
    return {read_fd(out), read_fd(err)};
}
...
private:
    ...
    int apply() const
    {
        ...
        const auto [out, err] = run("/home/quentin/Projects/bpfilter/build/output/bin/bfcli", args);
    ...
    int open()
    {
        ...
        while (true) {
            ...
            if (::std::string(info.name) == ::std::string("bf_benchmark")) {
                fd_ = prog_fd;
                return 0;
            }
...
public:
    ...
    int run(int expect, int iters, void *pkt, int len)
    {
        ...
        if (r < 0){
            ::std::cout << "Failed to run prog" << ::std::endl;
            return r;
            }
...
static void BM_test(benchmark::State& state) {
    ::bpfilter::Program p("bf_benchmark");
    p.open();

    for (auto _ : state) {
        p.run(2, 1, (void *)kLocalhostIPv6TCPPkt, sizeof(kLocalhostIPv6TCPPkt));
    }
}
BENCHMARK(BM_test);
